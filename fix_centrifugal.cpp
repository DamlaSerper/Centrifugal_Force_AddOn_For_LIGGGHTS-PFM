...
void FixCentrifugal::post_force(int vflag)
{
  ...
  double *radius = atom->radius; //~ New addition
  int *mask = atom->mask;
  ...
  double removaldist; //~ Add a margin to basketr so that the centrifugal force is applied continuously until the particle has fully egressed
  ...
  if (rmass) {
	...
	/* Apply the centrifugal force only to particles outside the inlet
	   pipe but within the basket, including a margin.*/
	removaldist = basketr + radius[i]; //~ Defining the margin
	if (r < removaldist && (r > inletpiperad || x[i][1] < inletpipey)) {	
	  f[i][0] += rmass[i]*omegasq*x[i][0];
